...
const ENTERPRISE_FILTERS = [
  ...
  { id: 'leadership', label: 'Leadership and sponsorship' },
  { id: 'communication', label: 'Communication and engagement' },
]
...
const SME_FILTERS = [
  ...
  { id: 'burnout', label: 'Staff burnout' },
  { id: 'customer', label: 'Customer experience' },
  { id: 'leadership', label: 'Leadership and accountability' },
]
  ...
  return (
    <Link href={tool.slug} className="block h-full">
      <motion.div layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
        ...
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center mb-4 transition-colors duration-200 ${iconBg}`}>
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <div className="flex flex-wrap gap-1.5 mb-3">
            {tool.outputs.slice(0,3).map(o => <span key={o} className="text-[11px] bg-[#F5F4F1] text-charcoal-3 px-2 py-0.5 rounded-full">{o}</span>)}
          </div>
...
export default function HubPage() {
  ...
  return (
    <>
      <Popup onComplete={() => setReady(true)} />

      <main className={`min-h-screen transition-opacity duration-300 ${ready ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>

        {/* Hero */}
        <section className="border-b border-border px-6 py-20 text-center">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            className="inline-flex items-center gap-2 bg-teal-light text-teal-dark text-[11px] font-medium tracking-widest uppercase px-4 py-1.5 rounded-full mb-8 border border-teal/20">
            <span className="w-1.5 h-1.5 rounded-full bg-teal animate-pulse" />
            Simply Clear · Clarity Suite
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
            ...
            Free diagnostics. Honest answers.
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            Built for leaders navigating enterprise transformation and small business complexity. No account. No noise.
          </motion.p>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
            className="text-sm text-charcoal-4 font-light italic">Adaptation, not adoption.</motion.p>
        </section>

        {/* Audience selector */}
        <section className="px-6 py-14 max-w-5xl mx-auto">
          <p className="text-sm font-medium tracking-widest uppercase text-teal text-center mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto mb-10">
            <button onClick={() => { setAudience('enterprise'); setFilter('all') }}
              ...
              <h3 className="font-serif text-lg text-charcoal mb-1">Enterprise</h3>
              <p className="text-sm text-charcoal-3 font-light">300 people and above</p>
            ...
            <button onClick={() => { setAudience('sme'); setFilter('all') }}
              ...
              <h3 className="font-serif text-lg text-charcoal mb-1">Small Business</h3>
              <p className="text-sm text-charcoal-3 font-light">Under 300 people</p>
        ...
        <AnimatePresence mode="wait">
          {audience && (
            <motion.section key={`${audience}-${filter}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        {!audience && (
          <div className="text-center py-8 text-charcoal-4 text-sm font-light">
            Select an audience above to see the right diagnostics for your situation.
          </div>
        )}
        ...
        <section className="bg-sand border-t border-border px-6 py-16 text-center mt-8">
          ...
          <p className="text-charcoal-3 font-light max-w-sm mx-auto mb-8 leading-relaxed">
            Short, honest thinking for leaders navigating significant change. No jargon. Delivered when we have something worth saying.
          </p>
          <a href="https://simplyclear.substack.com" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-teal text-white text-sm font-medium px-8 py-3.5 rounded-lg hover:bg-teal-dark transition-all mb-12">
            Subscribe to Brevity
          ...
          <div className="border-t border-border pt-8">
            <p className="text-charcoal-4 text-sm mb-2">
            ...
            <p className="text-xs text-charcoal-4 font-light">
              <a href="mailto:clarify@simplyclear.work" className="hover:text-charcoal transition-colors">clarify@simplyclear.work</a>
              <span className="mx-2">·</span>
              <a href="tel:+61410720730" className="hover:text-charcoal transition-colors">+61 410 720 730</a>
            </p>
          </div>
        </section>
      </main>
